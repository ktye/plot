...
void plotinit (double xmin, double xmax, double ymin, double ymax, const char *fgcolorstr, 
	...
	if ((width==0)||(height==0)) win = RootWindow(display, screen);
		...
		win = XCreateSimpleWindow(display,RootWindow(display,screen),
			10,10,width,height,0,BlackPixel(display,screen),WhitePixel(display,screen));
...
*/
/*
void handle_motion(XMotionEvent *event,double xmin,double xmax, double ymin, double ymax) {
...
void handle_key(XKeyEvent *event) {
	if (event->keycode == 24 || event->keycode == 9) exit(0);
}
...
//	XSelectInput(display, win, ExposureMask | KeyPressMask | ButtonPressMask | Button1MotionMask );
	XSelectInput(display, win, ButtonPressMask|KeyPressMask|ButtonReleaseMask );
	while (!done) {
...
//			break;
		case ButtonRelease:
			...
			break;
//		case MotionNotify:
//			handle_motion((XMotionEvent *)&event.xmotion,xmin,xmax,ymin,ymax);
//			break;
		case KeyPress:
			handle_key((XKeyEvent *)&event.xkey);
			break;
//		default:
//			printf("default\n");
//			break;
