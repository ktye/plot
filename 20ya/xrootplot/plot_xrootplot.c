...
void plotexit(void) {
	...
	if (filename) {
		...
		im_image = Imlib_create_image_from_drawable(im_data, pix, NULL, 0, 0, display_width, display_height);
		Imlib_save_image(im_data, im_image, filename, NULL);
	}
...
void plotinit (double xmin, double xmax, double ymin, double ymax, const char *fgcolorstr, 
	...
	if ((width==0)||(height==0)) win = RootWindow(display, screen);
		...
		win = XCreateSimpleWindow(display,RootWindow(display,screen),
			xoff,yoff,width,height,0,BlackPixel(display,screen),WhitePixel(display,screen));
